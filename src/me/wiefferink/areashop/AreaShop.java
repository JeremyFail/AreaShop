	...
	public void onEnable() {
		...
		if(!(plugin instanceof WorldEditPlugin) || !plugin.isEnabled()) {
			...
			try {
				...
				if (worldEdit.getDescription().getVersion().startsWith("7.2")) {
					weVersion = "7_2_X";
				} else {
					warn("Parsing the WorldEdit version failed, assuming version 7.2.X (some variant): ", rawWeVersion);
					weVersion = "7_2_X";
				}
		...
		if(!(plugin instanceof WorldGuardPlugin) || !plugin.isEnabled()) {
			...
			try {
				...
				if (worldGuard.getDescription().getVersion().startsWith("7.0.4")) {
					wgVersion = "7_0_4";
				} else {
					wgVersion = "7_0_4";
					warn("Parsing the WorldGuard version failed, assuming version 7.0.4: ", rawWgVersion);
				}
			} catch(Exception e) { // If version detection fails, at least try to load the latest version
				warn("Parsing the WorldGuard version failed, assuming version 7.0.4: ", rawWgVersion);
				wgVersion = "7_0_4";
